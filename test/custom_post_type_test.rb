...
class BookCPT < Compostr::CustomPostType
  wp_post_type "books"
...
class MovieCPT < Compostr::CustomPostType
  wp_post_type "movies"
...
class CPTTest < Minitest::Test
  ...
  def test_has_field
    ...
    #assert_equal true,  BookCPT.has_custom_field?("uuid")
  ...
  def test_diff_with_wordpress_unwhitespacing
    # spooky tag removal .... .gsub(/<\/?[^>]+>/, '') important for current_info
    # and
    #     re = /<("[^"]*"|'[^']*'|[^'">])*>/
    #         self.title.gsub!(re, '')
